...
1. docker-compose -f deployments/docker-compose.yml down
2. docker-compose -f deployments/docker-compose.yml build
3. docker-compose -f deployments/docker-compose.yml up -d


KAFKA
	- docker exec -it kafka1 /opt/kafka/bin/kafka-topics.sh --bootstrap-server kafka1:9092 --list
	- Consume recent messages:
		docker exec -i kafka1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka1:9092 --topic products --from-beginning --max-messages 10


Postgres — Connect and query
	- docker exec -it postgres-master psql -U admin -d products


Redis — Connect and inspect
	- docker exec -it redis redis-cli
